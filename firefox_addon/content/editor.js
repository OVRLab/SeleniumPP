...
var serverIP = 'http://localhost:7777';
///////////////////////////////////////////
...
function loadFormatter(url, type) {
...
var format = loadFormatter('chrome://seleniumideinterface/content/formats/webdriver.js', "webdriver");
Editor.prototype.addCommand = function (command, target, value, window, insertBeforeLastCommand) {
  ...
  var udpcommand = $("baseURL").value+"#$#"+command+"#$#"+target+"#$#"+value;
  var commandText = command;
  var command = new Command(command, target, value);
  var csharp_command = format.formatCommand(command);
  if(Array.isArray(target))
  {
    ...
    for (var i = 0; i < target.length; i++) {
      xtt[0] = target[i];
      target_sum += "#@#" + target[i];
      var cc = new Command(commandText, xtt, value);
      csharp_command += "#@#" + format.formatCommand(cc);
    };
    udpcommand = $("baseURL").value+"#$#"+commandText+"#$#"+target_sum+"#$#"+value;
  }
  // bind to the href attribute instead of to window.document.location, which
  // is an object reference
  command.lastURL = window.document.location.href;
  udpcommand += "#$#"+command.lastURL+"#$#"+csharp_command;
  //alert(udpcommand);
  xmlhttp.send(udpcommand);
  //////////////////////////////////////////////////////
...
Editor.prototype.appendWaitForPageToLoad = function (window) {
  ...
  if (lastCommand.type == 'command' && !lastCommand.command.match(/^(assert|verify|store)/)) {
    if (this.app.getCurrentFormat().getFormatter().remoteControl) {
      ...
      //     }
      // }
      // xmlhttp.send(lastCommand.command+"#$#"+
      //   lastCommand.target+"#$#"+
      //   lastCommand.value);
    }
  }
