...
namespace computer_assisted_instruction
{
    public static class ExtBitmap
    {
        ...
        private static byte[,] ConvolutionFilter2ByteArr(Bitmap sourceBitmap,
                                             double[,] filterMatrix,
                                                  double factor = 1,
                                                       int bias = 0,
                                             bool grayscale = false, int threshold = 0)
        {
            ...
            byte[,] result = new byte[sourceBitmap.Width, sourceBitmap.Height];
                                             ...
                                             bool grayscale = false,int threshold = 0)
        {
                ...
                sourceBitmap.Height - filterOffset; offsetY++)
            {
                    ...
                    sourceBitmap.Width - filterOffset; offsetX++)
                {
                    ...
                    if (blue > 255 - threshold)//255
                    { blue = 255; }
                    else if (blue <= 0 + threshold)//0
                    { blue = 0; }

                    if (green > 255 - threshold)//255
                    { green = 255; }
                    else if (green <= 0 + threshold)
                    { green = 0; }

                    if (red > 255 - threshold)//255
                    { red = 255; }
                    else if (red <= 0 + threshold)
                    { red = 0; }
        ...
        public static byte[,] Laplacian3x3Filter2ByteArr(this Bitmap sourceBitmap,
                                                    bool grayscale = true)
        {
            return ExtBitmap.ConvolutionFilter2ByteArr(sourceBitmap,
                                    Matrix.Laplacian3x3, 1.0, 0, grayscale
                                    , 50//Threshold of grayscale
                                    );
        }
